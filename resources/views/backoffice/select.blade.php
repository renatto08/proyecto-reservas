...
<head>
    ...
    <title>Seleccion de marca - Intermoda</title>
...
<div class="ibox login-content">
    <div class="text-center">
        <span class="auth-head-icon"><i class="la la-cart-plus"></i></span>
    </div>
    <form class="ibox-body pt-0" id="forgot-form" action="{{route('b.post.selectshop')}}" method="POST">
        {{csrf_field()}}
        <h4 class="font-strong text-center mb-4">PANEL DE PEDIDOS</h4>
        <p class="mb-4">Selecciona la marca que usarás.</p>
        <div class="form-group mb-4">
            <select name="shop" id="shop" class="form-control form-control-solid">
                @foreach($shops as $shop)
                    <option value="{{$shop->id}}">{{strtoupper($shop->name)}}</option>
                @endforeach
            </select>
        </div>
        <div class="text-center">
            <button class="btn btn-primary btn-rounded btn-block btn-air">Acceder</button>
        </div>
    </form>
</div>
